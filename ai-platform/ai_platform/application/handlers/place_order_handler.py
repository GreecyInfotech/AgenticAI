...
from ai_platform.application.commands.place_order import PlaceOrderCommand
...
class PlaceOrderHandler:
    ...
    async def handle(self, command: PlaceOrderCommand) -> OrderResponseDTO:
        ...
        for item in command.items:
            ...
            total += line_total
            await self._inventory.reserve(item.sku, item.quantity, order_id="pending")
        ...
        dto = to_order_dto(order)
        dto.status = ORDER_STATUS_CREATED
        return dto

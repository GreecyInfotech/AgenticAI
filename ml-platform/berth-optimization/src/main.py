"""ML inference service: berth-optimization."""
...
SERVICE_NAME = "berth-optimization"
SERVICE_PORT = 8301


class PredictionRequest(BaseModel):
    vessel_length: float = 0.0
    draft: float = 0.0
    cargo_type: float = 0.0
    priority: float = 0.0
...
class PredictionResponse(BaseModel):
    ...
    features_used: list[str] = Field(default_factory=lambda: ['vessel_length', 'draft', 'cargo_type', 'priority'])
...
async def lifespan(app: FastAPI):
    ...
    load_model()
    logger.info("ml_service_started", model="berth_optimizer")
    yield


app = FastAPI(title="berth-optimization", version="1.0.0", lifespan=lifespan)

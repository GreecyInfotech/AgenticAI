"""ML inference service: crane-failure-prediction."""
...
SERVICE_NAME = "crane-failure-prediction"
SERVICE_PORT = 8302
...
class PredictionResponse(BaseModel):
    ...
    features_used: list[str] = Field(default_factory=lambda: ['operating_hours', 'load_cycles', 'temperature', 'vibration'])
...
async def lifespan(app: FastAPI):
    ...
    load_model()
    logger.info("ml_service_started", model="crane_failure")
    yield


app = FastAPI(title="crane-failure-prediction", version="1.0.0", lifespan=lifespan)

...
class PredictionResponse(BaseModel):
    ...
    features_used: list[str] = Field(default_factory=lambda: ['metric_value', 'baseline', 'variance', 'time_window'])
...
async def lifespan(app: FastAPI):
    ...
    load_model()
    logger.info("ml_service_started", model="anomaly")
    yield


app = FastAPI(title="anomaly-detection", version="1.0.0", lifespan=lifespan)
